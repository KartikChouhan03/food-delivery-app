  ...
  const placeOrder = async (event) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Add the event listener for a user closing the payment dialog
        rzp.on('payment.closed', async function () {
        // This code runs if the user closes the popup without making a payment.
        try {
        await axios.post(url + "/api/order/verify", {
        orderId: newOrderId,
        success: "false" // Send 'false' to the backend
        }, { headers: { token } });
        } catch (error) {
        console.error("Error sending failed payment status to backend:", error);
        }
        window.location.href = `/verify?success=false&orderId=${newOrderId}`;
        });
        rzp.open();
        ...
        window.location.href = `/verify?success=false&orderId=null`;
      }
    } catch (error) {
